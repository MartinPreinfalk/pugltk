...
static constexpr char kFormat[] = "%.2f";
...
void ShowDemoWindow(bool *p_open) {
  if (ImGui::Begin("imgui-knobs-demo", p_open)) {
    if (ImGui::CollapsingHeader("Linear")) {
      ...
      // Push/PopStyleColor() for each colors used (namely ImGuiCol_ButtonActive and ImGuiCol_ButtonHovered for primary
      // and ImGuiCol_Framebg for Track)
      ...
      if (ImGuiKnobs::Knob("Dry", &val4, -6.0f, 6.0f, 0.1f, "%.1f", ImGuiKnobVariant_Stepped, 0, 0, 10, 1.570796f,
                           3.141592f)) {
        // value was changed
      }
      ...
      // Vertical drag only
      if (ImGuiKnobs::Knob("Vertical", &val6, 0.f, 10.f, 0.1f, "%.1f", ImGuiKnobVariant_Space, 0,
